...
また、次のような特定のサブタイプに注意を払うことも検討してください。

* `guava.bundle` - Guavaは、AEM 6.5 LTSでは標準でサポートされていないため、アップグレード後にバンドルを利用できません。
...
* `guava.bundle` サブタイプの場合は、Guavaをインストールするか、カスタムコードでGuavaを使用している場合は使用を削除します。
