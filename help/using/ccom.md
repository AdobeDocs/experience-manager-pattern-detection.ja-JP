...
>title="カスタムコンポーネント"
>abstract="CCOM は、AEMにインストールされているカスタムコンポーネントを特定します。 この情報は、ベストプラクティスの評価を目的として提供されます。"

`CCOM` AEMにインストールされているカスタムコンポーネントを特定します。 この情報は、ベストプラクティスの評価を目的として提供されます。

このコードのサブタイプは、コンポーネントのカテゴリの識別に使用されます。

* `custom.core`：コンポーネントのスーパータイプチェーン内のスーパータイプには、次のものが含まれます `core/wcm/components/`。コアコンポーネントから継承することを示します。
* `custom.foundation`：コンポーネントのスーパータイプチェーン内のスーパータイプには、が含まれます`core/wcm/components/`。コアコンポーネントから継承することを示します。
* `custom.overlay.foundation`：コンポーネントのパスにはが含まれます `wcm/foundation/components/` または `foundation/components/`、基盤コンポーネントをオーバーレイすることを示します。
